...
const userPreferences: Map<number, UserPreferences> = new Map();
...
export async function recordUserAction(
  ...
  if (!userPreferences.has(userId)) {
    userPreferences.set(userId, getDefaultPreferences());
  }
  ...
  switch (actionType) {
    case 'task_created':
      updateFromTaskCreation(preferences, actionData);
      break;
    case 'scheduling_accepted':
      updateFromSchedulingAcceptance(preferences, actionData);
      break;
    case 'priority_set':
      updateFromPrioritySetting(preferences, actionData);
      break;
    case 'time_estimated':
      updateFromTimeEstimation(preferences, actionData);
      break;
  }

  // Apply decay to old preferences
  applyPreferenceDecay(preferences);
 ...
 */
function updateFromTaskCreation(preferences: UserPreferences, taskData: any) {
  // Learn from task creation patterns
  if (taskData.time_of_day && ['morning', 'afternoon', 'evening'].includes(taskData.time_of_day)) {
    const timeOfDay = taskData.time_of_day as 'morning' | 'afternoon' | 'evening';
    preferences.time_preferences[timeOfDay] =
      (preferences.time_preferences[timeOfDay] || 0) + 0.1;
  }

  if (taskData.duration) {
    const durationCategory = getDurationCategory(taskData.duration);
 ...
 */
function updateFromSchedulingAcceptance(preferences: UserPreferences, schedulingData: any) {
  if (schedulingData.time_slot) {
    ...
    normalizePreferences(preferences.time_preferences);
  }
}
 ...
 */
function updateFromPrioritySetting(preferences: UserPreferences, priorityData: any) {
 ...
 */
function updateFromTimeEstimation(preferences: UserPreferences, timeData: any) {
  if (timeData.estimated_minutes) {
    const durationCategory = getDurationCategory(timeData.estimated_minutes);
    preferences.duration_estimates[durationCategory] =
      (preferences.duration_estimates[durationCategory] || 0) + 0.1;

    // Normalize duration estimates
    normalizePreferences(preferences.duration_estimates);
  }
}
 ...
 * Apply decay to old preferences
 */
function applyPreferenceDecay(preferences: UserPreferences) {
  // Apply decay to all preference values
  Object.keys(preferences.time_preferences).forEach(key => {
    const timeKey = key as keyof typeof preferences.time_preferences;
    preferences.time_preferences[timeKey] *= 0.98; // 2% decay
  });

  Object.keys(preferences.duration_estimates).forEach(key => {
    const durationKey = key as keyof typeof preferences.duration_estimates;
    preferences.duration_estimates[durationKey] *= 0.97; // 3% decay
  });
 ...
 */
function getTimeCategory(hour: number): 'morning' | 'afternoon' | 'evening' {
 ...
 */
function normalizePreferences(prefs: Record<string, number>) {
